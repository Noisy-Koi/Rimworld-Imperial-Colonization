...
namespace ImperialColonization
{
    public class QuestNode_GetPawn : QuestNode
    {
        ...
        public override bool TestRunInt(Slate slate)
        {
            ...
            if (canGeneratePawn.GetValue(slate) && (mustBeFactionLeader.GetValue(slate)
                || mustBeWorldPawn.GetValue(slate)
                || mustBePlayerPrisoner.GetValue(slate)
                || mustBeFreeColonist.GetValue(slate)))
            {
        ...
        private Pawn GeneratePawn(Slate slate, Faction faction = null)
        {
            ...
            Pawn pawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(result, faction, PawnGenerationContext.NonPlayer, -1, forceGenerateNewPawn: true, fixedTitle: fixedTitle));
